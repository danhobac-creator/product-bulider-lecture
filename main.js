document.addEventListener('DOMContentLoaded', () => {
    ...
    function getColor(number) {
        if (number <= 10) return '#f8d7da'; // 연한 노란색
        if (number <= 20) return '#d4edda'; // 연한 파란색
        if (number <= 30) return '#cce5ff'; // 연한 회색
        if (number <= 40) return '#fff3cd'; // 연한 빨간색
        return '#e2e3e5'; // 연한 녹색
    }
